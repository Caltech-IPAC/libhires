...
# This points to "DEV" version of hires, but most will want to use a numbered version.
...
def read_Planck_toast_file(filename, samples):
    ...
    for llist, blist, slist, alist in zip(glon, glat, signal, psi):
       for l, b, sig, angle in zip(llist, blist, slist, alist):
           x, y = projection.lonlat2xy(l, b)

           print -y, -x, sig, angle


           # samp = hires.Sample(-y, -x, sig, 1, 0.0)
           # samples.append(samp)
