...
#====================================================================
    ...
    def __init__(self, detector_id, iFloat, jFloat, angle, flux):
        ...
        full_response_array = self._get_response_array(detector_id, iFrac, jFrac, angle)
        untrimmed_image_bounds = \
                          self._compute_image_bounds(full_response_array, iWhole, jWhole)
...
#====================================================================
    ...
    # generate synthetic image if wanted
    synthetic_types = ('flat', 'spike', 'line', 'square', 'smooth')
    if filename not in synthetic_types: # read starting image
        global ITER_START
    ...
    else: # generate synthetic image
        image = np.zeros( (NPIXi, NPIXj), dtype=np.float32 )
        background = 0.0000001 
        foreground = background * 1000000.0
        image += background
        if filename == 'spike':
            image[NPIXi/2, NPIXj/2] = foreground
        if filename == 'line':
            for i in range(NPIXi/2-30, NPIXi/2+30):
                image[i, NPIXj/2] = foreground
        if filename == 'square':
            for i in range(NPIXi/2-30, NPIXi/2+30):
                for j in range(NPIXj/2-30, NPIXj/2+30):
                    image[i, j] = foreground
        if filename == 'smooth':
            for i in range(NPIXi/2-30, NPIXi/2+30):
                for j in range(NPIXj/2-30, NPIXj/2+30):
                    del_i = abs(i - NPIXj/2) -30
                    del_j = abs(j - NPIXj/2) -30
                    image[i, j] = background * float(del_i+del_j)
    return image
...
#====================================================================
    ...
    wgt_image = np.zeros( (NPIXi, NPIXj), dtype=np.float32)
...
#====================================================================
    ...
    flux_wgt_image = np.zeros( (NPIXi, NPIXj), dtype=np.float32)
    for foot in footprints:
        i0, i1, j0, j1 = foot.image_bounds
...
#====================================================================
    ...
    for foot in footprints:
        ...
        sim_flux = integration.sum() / foot.foot_sum
        foot.flux = sim_flux
...
#====================================================================
    ...
    spike_image = np.zeros( (NPIXi, NPIXj), dtype=np.float32)
...
# Initialize in case not set elsewhere
#  (must do outside get_parameters so can be overriden by read_one_IN_file)
FLUX_UNITS = '??'
...
#====================================================================
    ...
    print 'BOOST', boost_type, ' until ITER', BOOST_MAX_ITER
...
def main():
    ...
    if 'flux' in OUTFILE_TYPES:
        ...
        for iter in range(ITER_START+1, ITER_MAX+1):
            corr_wgt_image = calc_corr_wgt_image(all_footprints, flux_image, iter)
            flux_image *= corr_wgt_image / wgt_image
    ...
    if 'beam' in OUTFILE_TYPES:
        ...
        for iter in range(ITER_START+1, ITER_MAX+1):
            corr_wgt_image = calc_corr_wgt_image(all_footprints, beam_image, iter)
