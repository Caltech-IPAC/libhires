 ...
 const Binned_Data &binned_data)
{
  ...
  const double min_x(0), min_y(0), delta_bin_x(nxy[0]*radians_per_pix/bins),
    ...
    max_x(nxy[0]*radians_per_pix), max_y(nxy[1]*radians_per_pix);
              ...
              for(size_t ix_bin=0; ix_bin<bins; ++ix_bin)
                {
                  ...
                  double dx(x - bin_x), dy(y-bin_y),
                    dxm(x+bin_x), dxp(x-(2*max_x-bin_x)),
                    dym(y+bin_y), dyp(y-(2*max_y-bin_y));
                  response(ix_bin + bins*iy_bin,ix + nxy[0]*iy)=
                    radians_per_pix*radians_per_pix
                    *(exp(-(dx*dx+dy*dy)/(2*sigma_drf2))
                      + exp(-(dxp*dxp+dy*dy)/(2*sigma_drf2))
                      + exp(-(dxm*dxm+dy*dy)/(2*sigma_drf2))
                      + exp(-(dx*dx+dyp*dyp)/(2*sigma_drf2))
                      + exp(-(dx*dx+dym*dym)/(2*sigma_drf2))
                      + exp(-(dxp*dxp+dyp*dyp)/(2*sigma_drf2))
                      + exp(-(dxp*dxp+dym*dym)/(2*sigma_drf2))
                      + exp(-(dxm*dxm+dyp*dyp)/(2*sigma_drf2))
                      + exp(-(dxm*dxm+dym*dym)/(2*sigma_drf2)))/(2*pi*sigma_drf2);
                }
            }
        }
    }
