...
#====================================================================
    ...
    for ss in all_samples:
        angle_ss = ss.angle
        det_id_ss = ss.det_id
        if type(angle_ss) is float: angle_ss = iter.repeat(angle_ss)
        if type(det_id_ss) is int: det_id_ss = str(det_id_ss)
        if type(det_id_ss) is str: det_id_ss = iter.repeat(det_id_ss)
        # compute angle if needed
        # note: gets bogus values between legs of samples but usually off the image
        if ss.angle is None: # angle needs to be computed
            x_delta = ss.x[1:] - ss.x[0:-1] # x_next - x_this (for n-1 sample)
            y_delta = ss.y[1:] - ss.y[0:-1] # y_next - y_this (for n-1 sample)
            angle_radians = np.arctan2(y_delta, x_delta)
            angle_degrees = np.degrees(angle_radians)
            angle_ss = np.append(angle_degrees, angle_degrees[-1:]) # replicate last value

        for good, x, y, flux, angle, det_id in \
          zip(ss.good_ones, ss.x, ss.y, ss.flux, angle_ss, det_id_ss):
...
#====================================================================
    ...
    if filename == 'flat': # make a flat image to start
        ...
        if (kwd['CRVAL1'] != CRVAL1) or (kwd['CRVAL2'] != CRVAL2):
            log(LOG_error, "STARTING_IMAGE %s has inconsistent CRVAL value(s)\n" + \
