...
namespace hires
{
Hires::Hires (const std::string &Data_type, const std::string &Hires_mode,
              const std::vector<std::string> param_str)
    : starting_image ("flat"), beam_starting_image ("flat"), flux_units ("??"),
      log_filename ("logfile.log"), outfile_types{ "flux" },
      boost_max_iter (0), footprints_per_pix (1), beam_spike_n (5),
      ...
      beam_spike_height (10)
{
  ...
  if (data_types.find (Data_type) != data_types.end ())
    ...
    {
      std::stringstream ss;
      ss << "data_type is '" << Data_type << "' but must be one of ( ";
  ...
  std::map<std::string, Hires_Mode> hires_modes{
    { "hires", Hires_Mode::hires },
  ...
  if (hires_modes.find (Hires_mode) != hires_modes.end ())
    ...
    {
      std::stringstream ss;
      ss << "hires_mode is '" << Hires_mode << "' but must be one of ( ";
  ...
  for (int i = 0; i < param_str.size (); i++)
    {
      ...
      if (key == "SIZE_NPIX")
        ...
        throw Exception ("ERROR: Unknown parameter name: " + key);
    }
