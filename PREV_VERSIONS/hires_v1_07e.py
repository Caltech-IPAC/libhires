...
import numpy as np
import pyfits
from math import radians, degrees, cos, sin, atan2, sqrt, pow, pi
...
#====================================================================
    ...
    def __init__(self, detector_id, iFloat, jFloat, angle, flux, weight):
        ...
        bounds = Footprint._compute_footprint_ji(self.response_array, iWhole, jWhole)

        Footprint.j0min = min(Footprint.j0min, bounds[0])
        Footprint.j1max = max(Footprint.j1max, bounds[1])
        Footprint.i0min = min(Footprint.i0min, bounds[2])
        Footprint.i1max = max(Footprint.i1max, bounds[3])

        self.footprint_ji = bounds
...
#====================================================================
    ''' 1. Calculate padding adjustments to i,j bounds
        2. Apply to Footprint.footprint_ji
    '''

    global UNPAD_ji # ij bounds to get real image from padded image
    ...
    print ' DEBUG NPIXi NPIXj', NPIXi, NPIXj
    print ' DEBUG half_i half_j', half_i, half_j
    ...
    print 'i0_RAW', Footprint.i0min
    print 'i1_RAW', Footprint.i1max
    print 'j0_RAW', Footprint.j0min
    print 'j1_RAW', Footprint.j1max

    i0_padded = min(Footprint.i0min, i0_OUT)
    i1_padded = max(Footprint.i1max, i1_OUT)
    j0_padded = min(Footprint.j0min, j0_OUT)
    j1_padded = max(Footprint.j1max, j1_OUT)

    NPIXiPadded = i1_padded - i0_padded
    NPIXjPadded = j1_padded - j0_padded

    print 'i0_padded=', i0_padded
    print 'i1_padded=', i1_padded
    print 'j0_padded=', j0_padded
    print 'j1_padded=', j1_padded
    print 'NPIXiPadded=', NPIXiPadded 
    print 'NPIXjPadded=', NPIXjPadded 

    # Compute bounds of "real" sub-image within padded image
    i0_unpad = i0_OUT - i0_padded
    i1_unpad = i0_unpad + NPIXi
    j0_unpad = j0_OUT - j0_padded
    j1_unpad = j0_unpad + NPIXj

    print ' DEBUG UNPADs ji', j0_unpad, j1_unpad, i0_unpad, i1_unpad

    UNPAD_ji =  ( j0_unpad, j1_unpad, i0_unpad, i1_unpad )

    for foot in setOfFootprints:
        j0_foot, j1_foot, i0_foot, i1_foot = foot.footprint_ji
        new_ji =(j0_foot-j0_padded, j1_foot-j0_padded, i0_foot-i0_padded, i1_foot-i0_padded)

        # print "DEBUG new_ji", new_ji

        foot.footprint_ji = new_ji

#====================================================================
def calc_wgt_image(footprints):
#====================================================================
    ''' Create an image of all footprints *1.0 '''
    wgt_image = np.zeros( (NPIXjPadded, NPIXiPadded), dtype=np.float32)
    wgt_image += 0.0000001 # to avoid NaNs in results
    for foot in footprints:
        j0, j1, i0, i1 = foot.footprint_ji
        wgt_image[j0:j1, i0:i1] += foot.response_array * foot.weight
...
#====================================================================
    ...
    if filename == 'flat': # make a flat image to start
        image = np.zeros( (NPIXjPadded, NPIXiPadded), dtype=np.float32 )
        background = 1.0
        image += background
...
#====================================================================
    ...
    '''
    corr_wgt_image = np.zeros( (NPIXjPadded, NPIXiPadded), dtype=np.float32)
    if do_cfv: corr_sq_wgt_image = np.zeros((NPIXjPadded, NPIXiPadded),dtype=np.float32)
    else: corr_sq_wgt_image = None
    boost_mode = None
    for foot in footprints:
        j0, j1, i0, i1 = foot.footprint_ji
        integration = foot.response_array * flux_image[j0:j1, i0:i1]
        ...
        corr_wgt_image[j0:j1, i0:i1] += foot.response_array * correction
        if do_cfv:
            corr_sq = correction * correction
            corr_sq_wgt_image[j0:j1, i0:i1] += foot.response_array * corr_sq
...
#====================================================================
    ...
    for foot in footprints:
        j0, j1, i0, i1 = foot.footprint_ji
        integration = foot.response_array * sim_image[j0:j1, i0:i1]
...
#====================================================================
    ...
    spike_image = np.zeros( (NPIXjPadded, NPIXiPadded), dtype=np.float32)
    spike_image += 0.000001
    j0, j1, i0, i1 = UNPAD_ji
    iDelta = NPIXi / n
    jDelta = NPIXj / n
...
#====================================================================
    ...
    j0, j1, i0, i1 = UNPAD_ji
    if trim_padding:
        output_image = image[j0:j1, i0:i1] # trim off padding area
    else:
...
#====================================================================
    ...
    def lonlat2xy(self, lon_degrees, lat_degrees):
        lat = (pi/180)*(lat_degrees)
        lon = (pi/180)*(lon_degrees)
        cos_dlon = np.cos(lon - self.lon0)
        sin_dlon = np.sin(lon - self.lon0)
        sin_lat  = np.sin(lat)
        cos_lat  = np.cos(lat)
        cos_c = (pi/180)*( self.sin_lat0 * sin_lat + self.cos_lat0 * cos_lat * cos_dlon )
